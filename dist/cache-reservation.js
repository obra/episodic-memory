...
import { checkCacheCapacity } from './rclone-transport.js';
const RESERVATION_DIR = '.episodic-cache-reservations';
const REGISTRY_LOCK_WAIT_MS = 5_000;
export async function reserveCacheCapacity(cacheDir, localPath, nextBytes, freeBytes = () => {
    ...
    if (!handle)
        throw new Error('timed out waiting for cache reservation registry');
    try {
        ...
        const capacity = checkCacheCapacity(cacheSize(resolvedCache) + unmaterializedBytes, nextBytes, freeBytes() - unmaterializedBytes);
        if (!capacity.allowed)
            return { ...capacity, release: () => { } };
        const token = `${process.pid}.${randomUUID()}`;
        const reservationPath = path.join(reservationDir, `${token}.json`);
        const record = { token, pid: process.pid, bytes: nextBytes, localPath: resolvedLocal };
