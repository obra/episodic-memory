...
describe('search sidechain visibility', () => {
  ...
  describe('text-match contract (verbatim substring)', () => {
    beforeEach(async () => {
      await seed('proj', {
    ...
    it('matches a contiguous multi-word substring of a message', async () => {
      const results = await searchConversations('alpha beta gamma', { mode: 'text', limit: 10 });
      expect(results.length).toBe(1);
    });

    it('does not match when the query words are reordered (not a substring)', async () => {
      const results = await searchConversations('gamma alpha beta', { mode: 'text', limit: 10 });
      expect(results).toEqual([]);
    });

    it('does not match when the query interleaves words absent from the run', async () => {
